...
function [Xonp,p,Indices,idx_sep_seeds,Xonp_scrub] = CAP_find_activity(tcvox,...
    ...
    switch SeedType
        ...
        case 'Unisection'
            % Nothing needed anymore with the latest implementation
        otherwise
    ...
    if strcmp(SeedType,'Average')
        ...
        % for each subject.
        S = CAP_get_seed_traces(tcvox,logical(sum(seed,2)),SignMat(1,:));
    ...
    % different seeds need to be carried independently
        ...
        for idx = 1:size(seed,2)
            ...
            switch SeedType
                ...
                case 'Unisection'
                    ...
                    % For each seed, individual retained time points
                    tmp_seed_activity{idx} = cell2mat(xindp);
                    tmp_seed_scrub{idx} = cell2mat(flag_active);
            end
        ...
        % If we have the Unisection option, we still need to compute
        if strcmp(SeedType,'Unisection')
            
            % We'll operate differently depending on the number of seeds
            if size(seed,2) == 2
                
                tmp_union = tmp_seed_activity{1} | tmp_seed_activity{2};
                tmp_intersection = tmp_seed_activity{1} & tmp_seed_activity{2};
                Indices.kept.active = logical(tmp_union - tmp_intersection);
                
                tmp_union = tmp_seed_scrub{1} | tmp_seed_scrub{2};
                tmp_intersection = tmp_seed_scrub{1} & tmp_seed_scrub{2};
                Indices.scrubbedandactive = logical(tmp_union - tmp_intersection);
                
                
                
            elseif size(seed,2) == 3
                
                % For pairs of seeds
                tmp_intersection12 = tmp_seed_activity{1} & tmp_seed_activity{2};
                tmp_intersection23 = tmp_seed_activity{2} & tmp_seed_activity{3};
                tmp_intersection13 = tmp_seed_activity{1} & tmp_seed_activity{3};
                
                % For all the seeds together
                tmp_union123 = tmp_seed_activity{1} | tmp_seed_activity{2} | tmp_seed_activity{3};
                tmp_intersection123 = tmp_seed_activity{1} & tmp_seed_activity{2} & tmp_seed_activity{3};
                
                % At least two seeds jointly active
                tmp_atleasttwo = (tmp_intersection12 | tmp_intersection13 | tmp_intersection23 | tmp_intersection123);
                
                % All minus this, so that we only keep time points when
                % individual seeds are active
                Indices.kept.active = logical(tmp_union123 - tmp_atleasttwo);
                
                tmp_intersection12 = tmp_seed_scrub{1} & tmp_seed_scrub{2};
                tmp_intersection23 = tmp_seed_scrub{2} & tmp_seed_scrub{3};
                tmp_intersection13 = tmp_seed_scrub{1} & tmp_seed_scrub{3};
                
                tmp_union123 = tmp_seed_scrub{1} | tmp_seed_scrub{2} | tmp_seed_scrub{3};
                tmp_intersection123 = tmp_seed_scrub{1} & tmp_seed_scrub{2} & tmp_seed_scrub{3};
                
                tmp_atleasttwo = (tmp_intersection12 | tmp_intersection13 | tmp_intersection23 | tmp_intersection123);
                Indices.scrubbedandactive = logical(tmp_union123 - tmp_atleasttwo);
            else
                disp('This should never be entered!');
            end
        end
    end
