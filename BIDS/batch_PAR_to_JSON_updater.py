#!/usr/bin/env python3
"""
Multi-Site BIDS PAR-to-JSON Updater for NESDA fMRI Dataset

Author: JulianGaviriaL
Date: 2025-09-10 14:19:55

PURPOSE:
Automatically extracts MRI acquisition parameters from Philips PAR files and updates
BIDS-compliant JSON sidecars for fMRI preprocessing (e.g., fMRIPrep).

FEATURES:
- Multi-site support: Groningen (V4.1), Amsterdam (V4.2), Leiden (V4.2)
- Automatic site detection with confidence scoring
- Comprehensive BIDS parameter extraction (TR, TE, SliceTiming, PhaseEncoding, etc.)
- Philips-specific calculations: EffectiveEchoSpacing, slice timing (interleaved ascending)
- Batch processing with automatic backup creation
- Complete provenance tracking and metadata logging

CRITICAL CORRECTIONS:
- SliceTiming: Fixed for Philips interleaved ascending (odd slices first, then even)
- SliceEncodingDirection: Multi-strategy detection from orientation codes
- PhaseEncodingDirection: Extracted from Philips "Preparation direction" field

WORKFLOW:
Input -> Find PAR/JSON pairs -> Extract parameters -> Calculate derivatives -> 
Update JSONs -> Create backups -> Report statistics

USAGE:
python batch_bids_par_updater.py [/path/to/BIDS/root]
"""
    ...
    if version_match:
        ...
        if tool_version == '4.1':
            ...
            if examination_match and site_info['actual_site'] == 'unknown':
                ...
                if re.search(r'110\d{3}', exam_name):
...
def extract_philips_bids_3sites(par_file_path):
    ...
    try:
        ...
        print(f"          Site: {site_display} [confidence: {site_info['confidence']}]")
        ...
        if 'RepetitionTime' in bids_fields and n_slices:
            ...
            print(f"          Order: Odd slices (1,3,5,...,{2*((n_slices+1)//2)-1}), then even (2,4,6,...,{2*(n_slices//2)})")
        ...
        for pattern in recon_patterns:
            ...
            if match:
                ...
                if recon_x and recon_y:
                    ...
                    print(f"          ReconMatrix: {recon_x}x{recon_y}")
                    break
        ...
        if water_fat_shift and recon_matrix_pe:
            try:
                ...
                print(f"          ERROR: EffectiveEchoSpacing calculation failed: {e}")
        ...
        bids_fields['_ProcessingInfo'] = {
            ...
            'SliceTimingCorrected': '2025-09-10'
        }
        ...
        print(f"             - RepetitionTime: {'OK' if 'RepetitionTime' in bids_fields else 'MISSING'}")
        print(f"             - SliceEncodingDirection: {'OK' if 'SliceEncodingDirection' in bids_fields else 'MISSING'}")
        print(f"             - PhaseEncodingDirection: {'OK' if 'PhaseEncodingDirection' in bids_fields else 'MISSING'}")
        print(f"             - EffectiveEchoSpacing: {'OK' if 'EffectiveEchoSpacing' in bids_fields else 'MISSING'}")
        print(f"             - SliceTiming: {'OK' if 'SliceTiming' in bids_fields else 'MISSING'}")
...
def process_participant(participant_dir):
    ...
    if not func_dirs:
        print(f"      WARNING: No func directories")
        return 0, 0, 0
    ...
    for func_dir in func_dirs:
        ...
        print(f"      Directory: {participant_name}{session_info}/func")
        
        pairs = find_par_json_pairs(func_dir)
        
        if not pairs:
            print(f"        WARNING: No PAR/JSON pairs")
            continue
        
        total_pairs += len(pairs)
        
        for par_file, json_file in pairs:
            print(f"        Processing: {par_file.name} -> {json_file.name}")
            
            bids_params = extract_philips_bids_3sites(str(par_file))
            
            if not bids_params:
                print(f"          ERROR: No parameters extracted")
                total_failures += 1
                continue
            ...
            if success:
                print(f"          SUCCESS: Updated (added={added}, updated={updated})")
                total_successes += 1
            else:
                total_failures += 1
